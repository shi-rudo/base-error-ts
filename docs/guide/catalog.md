...
`metadata` is inferred per code and accepts JSON-safe data nested at most 100 levels deep (deeper metadata is rejected with "nested deeper than 100 levels"). A property whose value is `undefined` is skipped, as `JSON.stringify` skips it. `meta(code)` returns
