...
hero:
  ...
  tagline: A cross-environment base error class for TypeScript targeting Node.js, browsers and edge runtimes. A purely technical core plus a public-error pipeline for safe, localized public output that never leaks internal state by default.
  actions:
    - theme: brand
      text: Get Started
...
features:
  - title: Safe by default
    details: The core has no client serializer. Client-facing output is produced only by the public-error pipeline's explicit allowlist (a public code, an optional localized message, and any deliberately projected details), so internal state never leaks by accident.
  ...
  - title: Public error pipeline
    details: The public-error subpath turns a technical error into a curated, message-free PublicError, optionally localizes it, and maps it to framework-neutral RFC 9457 ProblemDetails, all from one descriptor per public code and without exposing the technical error.
