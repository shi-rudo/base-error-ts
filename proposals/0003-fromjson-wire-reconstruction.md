...
`fromJSON` is **not** an inter-service integration contract (see _Positioning_).
...
message, raw `details`). `fromJSON` reconstructs _that_ shape, so its honest home
...
1. **Bounded-context coupling.** Another service's `code`s belong to _its_
...
- **Lenient.** Missing structured fields fall back to safe defaults
  (`code "UNKNOWN_ERROR"`, `category "INTERNAL"`, `retryable false`), using the same
...
3. **Don't trust reconstructed fields for authorization.** An attacker who can
   ...
   `fromJSON` to reconstruct _shape_ for handling/logging, not as an authority
...
5. **Versioning.** No `version` field is proposed; the lenient field-by-field
   ...
   evolution. A versioned schema is the _consumer's_ concern at a real
...
- Not a general-purpose deserializer/validator (it reconstructs _this_ library's
  serialized shape).
