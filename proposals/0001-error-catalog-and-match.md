...
- If `err`'s type is a **closed union** of error types with literal `code`s and
  ...
  matchError(err, {
    USER_NOT_FOUND: () => 404,
    _: (e) => { log(e.code); return 500 },   // catch-all opt-out
  });
  ```
...
- **Per-case narrowing needs a real union.** `Extract<E, { code: K }>` only
  narrows `details` etc. when `E` is a *union of distinct* error types (the
...
- **Instances vs nominal classes.** Generating one `StructuredError` instance
  ...
  `matchError`. Generating a distinct *class* per code would add per-code
...
function toResponse(err: AppError) {
  return matchError(err, {
    USER_NOT_FOUND: (e) =>
      e.toProblemDetails({ status: 404, locale: "en" }),
    RATE_LIMITED: (e) =>
      e.toProblemDetails({
        status: 429,
