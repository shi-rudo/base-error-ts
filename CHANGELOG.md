...
- **Optional contract inspection for consumer tests.** `inspectOwnLogFields(record)` reports reserved names, unsupported values, accessors, cycles, and bounds through `OwnLogFieldsIssue` paths. It does not execute getters or serialization callbacks and never runs implicitly during logging.

- **`OwnLogFields` describes data contributed by a subclass.** Import this type for a `buildOwnLogFields()` override or a `satisfies` check. It describes a readonly record whose values are JSON primitives, readonly arrays, or nested records. JSON primitives include `null`. Convert dates, collections, and bigints explicitly. The base hook retains its broader signature for source compatibility. Runtime guards remain necessary.

- **A cause keeps the fields its own class declares.** `buildLogObject()` was the extension point for a subclass's own fields, and that promise held for an error logged directly and broke for the same error one level down, because a cause node was assembled from a fixed key roster. A wrapper at a boundary is the normal case, so the wrapping decided whether the data survived, not the error class: a responder held the error code but not the fields the code refers to. A subclass now declares its fields by overriding the new `buildOwnLogFields()`, and those fields appear at every depth of every chain that wraps the error. The fields a class declares for itself are composed separately from the hook, so `StructuredError` keeps `code`, `category`, `retryable` and `details` whatever a subclass returns, and its diagnostic fields remain available. The fixed roster stays as the only route for a foreign cause, which has no reachable hook.

  The hook is consumer code, and it runs on a path that executes inside `catch`. The library catches exceptions and limits the fields it processes. It cannot interrupt consumer code; the hook must finish synchronously. A key that carries a name this library writes is dropped rather than obeyed, so a field named `category` or `details` cannot survive an allow-list. A node carries at most `MAX_OWN_LOG_FIELDS` (100) own fields, and the reader stops there. A throw, or a return that is not a record, costs the fields and never the node. A getter in the hook record that throws leaves `[Unserializable value]` at its own key. A field whose value is an error is dropped, because logging it would re-enter the log build. Every value is copied as data, at the root and on a cause alike, so the log shares no reference with the error, and a bigint or a cycle cannot make the consumer's `JSON.stringify` throw.

  The hook runs once per serialized node, after its fixed fields. Its fields therefore land after the fields a class declares for itself, and a responder still finds `code` near the front of the line.

  Rejected keys, hook failures, and field-width cuts remain silent. Exhausting the shared log budget replaces the current field with `[Max log size exceeded]`; later fields may be cut too. This is a value, not an extra diagnostic key, and normal data redaction applies.

  `StructuredError.fromJSON` does not restore subclass fields; it reconstructs the envelope it whitelists, and a round-trip test pins that.
...
- **`project()` copies the fault values that it checked.** It checked `field` and `code` through a guarded read and then read both a second time for the copy. A getter that returned a string first and a number second put the number into the frozen view, and the outcome still reported `projection: "succeeded"`. Each value is now read once, and `project()` and `toProblem` share one copy of the fault shape. A hole in the returned list now fails the list, like any other malformed fault.

- **The JSON-safe clone builds every list as a plain array.** It cloned a list with `map`, which constructs the result through the source's `Symbol.species`. An `Array` subclass with its own `toJSON` rewrote the `details` member of a `toProblem` body, and an own `constructor` key on a plain array replaced the clone with any object. The clone now copies a list index by index into a fresh array. A list whose prototype is not `Array.prototype` is rejected, as an object with a foreign prototype already is. This also covers `extensions` and `defineErrors` metadata.

- **An `undefined` property no longer rejects a whole JSON-safe value.** One optional field left `undefined`, for example `projectDetails: (e) => ({ id: e.id, hint: e.hint })`, dropped all of `details` from the `toProblem` body. The same held for `extensions`, and `defineErrors` threw for such a `metadata` property. The types of all three admit optional properties. The shared clone now skips an `undefined` property, as `JSON.stringify` does. An `undefined` list element is still rejected, because JSON turns it into `null`. A `metadata` property that is `undefined` by mistake no longer fails at definition time; it is absent from `meta(code)`.
...
- **`partialMask` masks the whole value for an invalid `keepStart` or `keepEnd`.** A negative count inverted the too-short guard and the slice arithmetic: `partialMask({ keepStart: 4, keepEnd: -4 })` returned the full value, and `keepStart: -1` revealed all but one character. `NaN` and a non-integer count reached the slice calls unchecked. Both counts are validated at construction, and an invalid count (negative, `NaN`, infinite, or not an integer) yields a mask that returns the fill for every value. A full mask instead of a throw, because the mask runs on the logging path and `partialMask` is commonly called inside an error constructor, which is a catch path.

- **`redactAllow` treats a container under an envelope name as data.** The region rule kept any container found under `name`/`message`/`stack`/`code`/`category`/`retryable` in the cause region, at every depth, so a cause with `stack: { message: "Bearer …" }` or `code: { message: … }` kept those leaves verbatim under an allow-list that listed nothing. An envelope key holds a primitive. A container under an envelope name is data now, at the root and inside a cause, and its leaves are masked whatever they are named. Only `cause` and `errors` stay on the cause spine, and only `details` opens a data subtree.

- **The redaction walker drops function-valued leaves.** The deny-list copied an own `toJSON` function under `details` into the masked clone as an ordinary leaf, so the consumer's `JSON.stringify` called it and re-materialized the masked key in full. A function is never log data: in both modes the walker now omits a function-valued property and writes a function array item as `null`, which is what `JSON.stringify` does, so the clone is JSON-shaped and carries no `toJSON`.

- **A deny-listed `message` is masked in the `stack` fields of the log object too.** `redact(["message"])` masked the `message` field, but the same log object carries `stack`, whose header line repeats the raw message, on the error itself and on every cause, so `JSON.stringify(err)` still contained the text that the pitfalls guide marked as redacted. When the deny-list names `message`, the header of every `stack` on the cause spine (the error, each nested cause, each aggregate member) is now rewritten with the masked message, and the frames stay. A `stack` that does not start with the exact `name: message` header, as a foreign cause on a runtime that writes no header, is handed to the mask as a whole, because the library cannot prove that the message is absent from it. The `err.stack` property and `console.log(err)` inspection stay raw, as documented.

- **The class guards answer `false` for a value that cannot be inspected.** `isBaseError`, `isErrorOf`, `isAnyErrorOf`, `matchThrown().with`/`.withAny`, `defineErrorClassSet().match` and a catalog's `is` ran a bare `instanceof`, which throws on a revoked Proxy or on a Proxy whose `getPrototypeOf` trap throws, while `isError` and `isStructuredError` in the same module already caught that input. Every class guard now runs the built-in check through one guarded helper and answers `false` for such a value. A constructor with its own `Symbol.hasInstance` is the caller's code and still propagates.

- **`publicIssues()` projects a `null` path segment instead of throwing.** The pointer builder read `.key` from every object segment, and `null` is an object: an issue with `path: ["user", null, "email"]` threw a `TypeError` out of `publicIssues()`, on the client boundary path. A `null` segment is now rendered as the pointer segment `null` and stays `null` in the projected `path`, matching the path copy.

- **A native cause's extension fields are copied as data in the log object.** The native-Error branch of the cause serializer copied `code`, `category`, `retryable` and `details` (and a non-string `name`/`message`/`stack`) by reference, without the bounded data copy that a plain-object cause gets, so `JSON.stringify(err)` in the consumer's logger threw for a foreign cause whose `code` is a bigint or whose `details` holds a bigint, a cycle or a throwing `toJSON`, and a mutation of the cause after logging changed a log object already built. Both branches now share one data rule: a bigint becomes its decimal string at every depth (a nested bigint no longer degrades the whole object to the marker, on the plain-object path too), a function or symbol is absent, and an object is decoupled by a bounded walker and replaces a repeated ancestor reference with a bounded `[Circular …]` description. An unreadable serialization callback becomes `[Unserializable value]` at its own position. A `Date` under a cause's `details` is its ISO string in `toLogObject()`; the JSON output is unchanged. A cause whose `details` is a Proxy with throwing traps keeps its envelope with `details: "[Unserializable value]"` instead of collapsing to `[Unserializable cause]`. The error's own `details` stay raw.

- **Aggregate members are read through the guarded reader as a bounded copy.** The member reader probed `Array.isArray` unguarded, which throws on a revoked Proxy, and handed the live foreign `errors` array to `toString()`, `toLogObject()`, `StructuredError.fromJSON` and the tree traversal, which then read `length`, an index or `Symbol.species` on it outside any guard: a throwing `length` trap, a throwing index getter or an array subclass with a throwing species made all four throw, and a `length` of `2**53` made the traversal hang. Every reader now receives a fresh frozen copy of at most its own bound (the aggregate width for the serializer, the string render and `fromJSON`, the remaining node budget for the traversal), read element by element through guarded reads, together with the guarded `length` as the count behind the `(+N aggregated)` suffix and the `[N more aggregated errors]` marker. A `length` that is not a non-negative safe integer reads as zero members. For well-formed input the output is unchanged. One tightening: the traversal reads at most its remaining budget of members per aggregate, so members past a spent budget are never visited.

- **`cloneJsonSafe` rejects a container nested deeper than 100 levels.** The clone-and-freeze walker behind `defineErrors` and `toProblem` carried a node budget and a cycle guard but no depth cap, so a 2000-deep array overflowed the host stack: `defineErrors` surfaced a raw `RangeError` instead of "metadata must be JSON-safe", and a deep object was accepted whole on a host with a large stack and rejected on a smaller one. A container at the 101st level is now rejected like every other value that is not JSON-safe, with the reason named because the value itself passes `JSON.stringify`: `defineErrors` throws its documented message followed by "(nested deeper than 100 levels)", and `toProblem` omits the member and names it in `outcome.omitted`. The first 100 levels clone in full and are frozen at every level.

- **`toString()` cuts a linear cause chain where the log object cuts it.** The string render bounded aggregate nesting but walked a linear chain without a hop limit, so a chain of 20 000 errors rendered 20 002 lines while `toLogObject()` stopped at 100 cause nodes. `toString()` now renders at most 100 cause nodes and ends with `Caused by: [Max cause depth exceeded]`, counting hops exactly as the serializer does, so for a chain of error objects a member's chain is cut at the same node on both surfaces. A plain-object cause is one data value in the log object, copied whole with no cause cap inside it, while `toString()` follows its `cause` links and ends them at the same cap. A chain of any length costs at most 100 property reads. Aggregate nesting is cut at depth 100 instead of 101.

- **The redaction walker caps the cause spine and marks a cycle once.** The walker kept the cause spine (`cause` links and `errors` members) out of its data depth cap so that a deep chain would not truncate a shallow `details`, but nothing else bounded that spine: a plain-object cause chain 3000 deep, or a deep object a subclass put on the spine, overflowed the host stack under any redactor and the whole log collapsed to `[log redaction failed]`, on an edge isolate far earlier. The walker also had no seen set, so a back-reference in the error's own `details` was cloned 100 levels deep before the depth marker stopped it (an order with ten line items that point back at it produced 12 MB per log line). The spine now carries its own hop counter with the same cap as the serializer, past which a container becomes `[Max redaction depth exceeded]`, and a container already on the current path becomes `[Circular reference]` at its first repeat. A shared reference without a cycle is still cloned per reference within the node budget, as documented. That budget now counts every value the walk visits in a data region, leaves included, as the other two data walkers do: under a redactor a data tree with more than 100 000 values ends with `[Max redaction size exceeded]` in place of the rest, in key order, while the root and cause envelopes are never cut by size and the raw object without a redactor is unchanged. The data depth still restarts at each cause node.
...
- **`StructuredError.fromJSON` keeps its never-throws contract against in-process payloads.** The documentation promises that malformed input yields the fallback envelope, but every payload field was read with bare property access, and the shallow copy of `details` invoked arbitrary getters: a payload with a throwing getter (safe for `JSON.parse` output, common for values handed across an in-process boundary) made `fromJSON` throw. Every field of the payload and of every nested cause node is now read through the guarded reader, and a `details` object whose own getter throws during the copy is dropped while the envelope survives.

- **`StructuredError.fromJSON` carries a total node budget.** Reconstruction bounded depth (100) and the members per aggregate (100), but not their product: a payload with a 100-wide aggregate nested three deep reconstructs a million `Error` objects, each capturing a stack (measured at 3 s), and a shared reference makes such a payload a few hundred bytes in memory. One `fromJSON` call now reconstructs at most 10,000 cause nodes: enough for every realistic log shape to round-trip losslessly, while a hostile payload is capped at tens of milliseconds. The serializer itself has no total-node cap (its depth and width caps compound), so no finite budget covers every legal output: a pathologically nested aggregate (hundreds of aggregates inside aggregates) still truncates. Past the budget, a `cause` drops exactly as it does past the depth cap, and the remaining members of an aggregate collapse into the `[N more aggregated errors]` marker.
