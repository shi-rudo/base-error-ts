...
- **Three guard-then-raw-read gaps closed.** `toStructuredError` read `value.toLogObject` and `isRetryableStructuredError` read `value.retryable` unguarded right after their fail-closed guards, and `readErrorCode`'s `"code" in error` probe sat outside its try block, so a throwing getter, a stateful getter, or a Proxy with a throwing `has` trap threw out of coercion, retry decisions, and catalog resolution (`project` lost its totality). All three now read through the guarded reader.
