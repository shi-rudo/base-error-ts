...
- **A cause keeps the fields its own class declares.** `buildLogObject()` is documented as the extension point for a subclass's own fields, and that promise held for an error logged directly and broke for the same error one level down, because a cause node was assembled from a fixed key roster. A wrapper at a boundary is the normal case, so the wrapping decided whether the data survived, not the error class: a responder held the error code but not the fields the code refers to. A subclass now declares its fields by overriding the new `buildOwnLogFields()`, and those fields appear at every depth of every chain that wraps the error. The fields a class declares for itself are composed separately from the hook, so `StructuredError` keeps `code`, `category`, `retryable` and `details` whatever a subclass returns, and its own output is unchanged. The fixed roster stays as the only route for a foreign cause, which has no reachable hook.
  ...
  Every loss on this path is silent. It writes no marker of its own, unlike the cause spine, because a marker here is a key that a redaction region has to classify and that a subclass could forge.
...
- **A native cause's extension fields are copied as data in the log object.** The native-Error branch of the cause serializer copied `code`, `category`, `retryable` and `details` (and a non-string `name`/`message`/`stack`) by reference, without the bounded JSON round-trip that a plain-object cause gets, so `JSON.stringify(err)` in the consumer's logger threw for a foreign cause whose `code` is a bigint or whose `details` holds a bigint, a cycle or a throwing `toJSON`, and a mutation of the cause after logging changed a log object already built. Both branches now share one data rule: a bigint becomes its decimal string at every depth (a nested bigint no longer degrades the whole object to the marker, on the plain-object path too), a function or symbol is absent, and an object is decoupled by the bounded round-trip and degrades to the `[Circular …]` marker where it has no JSON form. A `Date` under a cause's `details` is its ISO string in `toLogObject()`; the JSON output is unchanged. A cause whose `details` is a Proxy with throwing traps keeps its envelope with `details: "[Circular Object]"` instead of collapsing to `[Unserializable cause]`. The error's own `details` stay raw.
