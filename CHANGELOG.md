...
- Added `redact(keys, { mask? })` and `redactWith(fn)` — sticky, opt-in PII redaction on the **log** path (`toLogObject`/`toJSON`), so even a logger's `JSON.stringify(error)` is masked. `redact` deep-masks matching keys (default mask `"[REDACTED]"`, configurable); `redactWith` transforms the whole log object (allow-lists, message scrubbing). The client serializers are unaffected (already safe). Defense-in-depth, not a replacement for logger-level redaction.
