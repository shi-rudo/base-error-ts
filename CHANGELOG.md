...
- **A cause keeps the fields its own class declares.** `buildLogObject()` is documented as the extension point for a subclass's own fields, and that promise held for an error logged directly and broke for the same error one level down, because a cause node was assembled from a fixed key roster. A wrapper at a boundary is the normal case, so the wrapping decided whether the data survived, not the error class: a responder held the error code but not the fields the code refers to. A subclass now declares its fields by overriding the new `buildOwnLogFields()`, and those fields appear at every depth of every chain that wraps the error. The fields a class declares for itself are composed separately from the hook, so `StructuredError` keeps `code`, `category`, `retryable` and `details` whatever a subclass returns, and its own output is unchanged. The fixed roster stays as the only route for a foreign cause, which has no reachable hook.

  The hook is consumer code, and it runs on a path that executes inside `catch`. The library catches exceptions and limits the fields it processes. It cannot interrupt consumer code; the hook must finish synchronously. A key that carries a name this library writes is dropped rather than obeyed, so a field named `category` or `details` cannot survive an allow-list. A node carries at most `MAX_OWN_LOG_FIELDS` (100) own fields, and the reader stops there. A throw, or a return that is not a record, costs the fields and never the node. A getter that throws costs its own key only. A field whose value is an error is dropped, because logging it would re-enter the log build. Every value is copied as data, at the root and on a cause alike, so the log shares no reference with the error, and a bigint or a cycle cannot make the consumer's `JSON.stringify` throw.

  The hook runs once per log object, after the whole subclass chain. Its fields therefore land after the fields a class declares for itself, and a responder still finds `code` near the front of the line.
  ...
  Overriding `buildLogObject()` keeps working during migration. `StructuredError.fromJSON` does not restore subclass fields; it reconstructs the envelope it whitelists, and a round-trip test pins that.
...
- **Empty overrides keep the original diagnosis.** An empty or all-undefined `buildLogObject()` return falls back to the base envelope, including stack and bounded cause, rather than reporting a build failure (`base-error-ts-arj`). The copy retains non-enumerable own envelope values. Root property order and undefined `stack`/`cause` slots match the original base envelope (`base-error-ts-pev`).
...
- **A throwing subclass override no longer escapes the logging path.** `toLogObject()` called `buildLogObject()` outside any guard, and only the redactor below it was wrapped, so a subclass whose override threw (a throwing getter on one of its own fields is the realistic form) threw out of `toLogObject()`, `toJSON()` and the consumer's `JSON.stringify`. That happens inside a `catch`, where a new exception destroys the error the caller set out to log, against the totality contract of this path. The call is now guarded. The fallback keeps what it can: the envelope this class builds carries `name`, `message`, `stack` and the bounded cause chain, and no subclass contributed to it. The structural fields a subclass declares are read from the instance for that envelope, so the milder failure no longer loses the machine-readable `code` that the deeper triage envelope keeps. When that envelope throws as well, a field of the instance itself is hostile, and the guarded triage envelope stands in with the message `"[log build failed]"`. An override that returns a primitive, an array or nothing counts as a failure too, so `toLogObject()` always returns a record.
...
- **A native cause's extension fields are copied as data in the log object.** The native-Error branch of the cause serializer copied `code`, `category`, `retryable` and `details` (and a non-string `name`/`message`/`stack`) by reference, without the bounded data copy that a plain-object cause gets, so `JSON.stringify(err)` in the consumer's logger threw for a foreign cause whose `code` is a bigint or whose `details` holds a bigint, a cycle or a throwing `toJSON`, and a mutation of the cause after logging changed a log object already built. Both branches now share one data rule: a bigint becomes its decimal string at every depth (a nested bigint no longer degrades the whole object to the marker, on the plain-object path too), a function or symbol is absent, and an object is decoupled by a bounded walker and uses the legacy `[Circular …]` fallback for a cycle or an unreadable serialization callback. A `Date` under a cause's `details` is its ISO string in `toLogObject()`; the JSON output is unchanged. A cause whose `details` is a Proxy with throwing traps keeps its envelope with `details: "[Circular Object]"` instead of collapsing to `[Unserializable cause]`. The error's own `details` stay raw.
