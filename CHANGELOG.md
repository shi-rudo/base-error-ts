...
- **`toString()` cuts a linear cause chain where the log object cuts it.** The string render bounded aggregate nesting but walked a linear chain without a hop limit, so a chain of 20 000 errors rendered 20 002 lines while `toLogObject()` stopped at 100 cause nodes. `toString()` now renders at most 100 cause nodes and ends with `Caused by: [Max cause depth exceeded]`, counting hops exactly as the serializer does, so a member's chain is cut at the same node on both surfaces. A chain of any length costs at most 100 property reads. Aggregate nesting is cut at depth 100 instead of 101.
